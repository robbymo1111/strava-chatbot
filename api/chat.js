...
module.exports = async (req, res) => {
  ...
  try {
    ...
    // ── Memory-update diagnostic ──────────────────────────────────────────────
    // Log whether Claude generated a <memory-update> block so we can diagnose
    // pipeline breaks: this log should appear whenever the athlete tells the coach
    // something worth remembering (goal, PR, injury, calf tightness, etc.)
    const memUpdateMatch = rawReply.match(/<memory-update>([\s\S]*?)<\/memory-update>/);
    console.log('[memory-update] tag generated by Claude:', !!memUpdateMatch);
    if (memUpdateMatch) {
      try {
        const parsed = JSON.parse(memUpdateMatch[1].trim());
        console.log('[memory-update] valid JSON — goals:', (parsed.goals||[]).length,
          'prs:', (parsed.prs||[]).length, 'injuries:', (parsed.injuries||[]).length,
          'notes:', (parsed.notes||[]).length, 'maxHR:', parsed.maxHR);
      } catch (e) {
        console.warn('[memory-update] JSON parse failed:', memUpdateMatch[1].substring(0, 200));
      }
    }

    // ── Build reply for the client ────────────────────────────────────────────
    // IMPORTANT: keep <memory-update> in the client reply so the frontend's
    // extractMemoryUpdate() can parse + save it.  Only strip <session-note>
    // (backend-only; the client should never see it).
    const reply = rawReply
      .replace(/<session-note>[\s\S]*?<\/session-note>/g, '')
      .replace(/<session-note>[\s\S]*/g, '')   // catch truncated (no closing tag)
      .trim();

    // Strip BOTH tags for storage — chat history should be clean text only
    const replyForStorage = rawReply
      .replace(/<session-note>[\s\S]*?<\/session-note>/g, '')
...
async function attachLapsToWorkouts(activities, accessToken) {
  ...
  // Quality detection for laps AND splits.
  // Criteria: pace < 8:30/mi, distance > 6mi, or workout/race type.
  // Slightly broader than the old threshold (was 8:00/mi, 10mi) so mile-split
  // data is available for longer easy runs and mid-length quality sessions.
  function meetsLapCriteria(a) {
    ...
    return avgMPM < 8.5 || distMi > 6 || labeled || suffer > 40 || maxHR > 155;
  }
  ...
  if (kvUrl && kvToken && athleteId) {
    ...
    candidates.forEach((a, i) => {
      ...
      if (cached && cached.v === 2 && cached.laps && cached.laps.length > 1) {
        ...
        a._lapStatus   = 'cached';
        // Prefer splits_standard stored in the lap entry over stream-computed splits
        if (cached.splits?.length > 0) a._mileSplits = cached.splits;
      } else if (cached && cached.v === 2 && cached.splits?.length > 0) {
        // Activity has been fetched before: no manual laps but has splits_standard data.
        // Don't re-fetch — the empty laps array is the confirmed result from Strava.
        a._lapAnalysis = cached;
        a._lapStatus   = 'cached';
        if (!a._mileSplits) a._mileSplits = cached.splits;
      } else {
  ...
  for (let bStart = 0; bStart < toFetch.length; bStart += 5) {
    ...
    await Promise.all(batch.map(async (a) => {
      try {
        // Parallel-fetch laps AND activity detail (for splits_standard).
        // Both count against Strava rate limit but only run on cache misses.
        const [rLaps, rDetail] = await Promise.all([
          fetch(`https://www.strava.com/api/v3/activities/${a.id}/laps`,
            { headers: { Authorization: `Bearer ${accessToken}` } }),
          fetch(`https://www.strava.com/api/v3/activities/${a.id}`,
            { headers: { Authorization: `Bearer ${accessToken}` } }),
        ]);

        // Extract splits_standard from activity detail
        let splitsStandard = null;
        if (rDetail.ok) {
          try {
            const detail = await rDetail.json();
            splitsStandard = detail.splits_standard || null;
            console.log(`[splits] activity ${a.id}: splits_standard count=${splitsStandard?.length ?? 0}`);
          } catch (_) {}
        }
        const convertedSplits = convertSplitsStandard(splitsStandard);
        if (convertedSplits?.length > 0) a._mileSplits = convertedSplits;

        if (!rLaps.ok) { a._lapStatus = 'error'; return; }
        const rawLaps = await rLaps.json();

        if (!Array.isArray(rawLaps) || rawLaps.length < 2) {
          a._lapStatus = 'insufficient';
          // Still store splits even when no manual laps
          if (convertedSplits?.length > 0 && kvUrl && kvToken && athleteId) {
            kvWrite(kvUrl, kvToken, `laps:${athleteId}:${a.id}`, {
              v: 2, activityId: a.id, laps: [], pattern: null,
              hardEffortSummary: null, splits: convertedSplits,
              source: 'chat-live-fetch', analyzedAt: Date.now(),
            });
          }
          return;
        }
        ...
        const entry = {
          ...
          splits: convertedSplits || null,  // store splits_standard alongside laps
        };
        a._laps       = rawLaps;
...
/**
 * Convert Strava splits_standard array to the internal per-mile split format.
 * splits_standard is from GET /activities/{id} — Strava's own pre-processed
 * per-mile splits including grade-adjusted pace. More reliable than
 * stream-computed splits.
 *
 * Input fields (per split):
 *   split, distance(m), average_speed(m/s), average_heartrate,
 *   elevation_difference(m), average_grade_adjusted_speed(m/s)
 *
 * Output matches what formatMileSplits() expects:
 *   { mile, pace, gap, hr, elevFt, effort, paceMPM }
 */
function convertSplitsStandard(splitsStandard) {
  if (!Array.isArray(splitsStandard) || !splitsStandard.length) return null;

  // Compute peak HR across all splits for effort classification
  ...
  const result = splitsStandard.map(s => {
    ...
    if (avgSpeed < 0.5) return null; // skip near-zero speed splits (GPS pause)
    ...
    const pace = fmtPace(paceMinPerMile);
    const gap  = (Math.abs(gapMinPerMile - paceMinPerMile) > 0.05) ? fmtPace(gapMinPerMile) : null;

    return { mile: s.split, pace, gap: gap || pace, hr, elevFt, effort, paceMPM: paceMinPerMile };
  }).filter(Boolean);
...
/**
 * Format per-mile split data (from splits_standard or stream compute) for the prompt.
 * Detects split trend (negative/positive/even) and HR drift.
 *
 * Output example:
 *   Mile splits (Strava · negative split):
 *     Mile 1: 7:42/mi | HR 148 | elev +42ft | GAP 7:38
 *     Mile 2: 7:38/mi | HR 152 | elev +18ft | GAP 7:35
 */
function formatMileSplits(splits) {
  if (!splits || splits.length === 0) return '';

  // Detect pace trend across all splits
  const paces = splits.map(s => s.paceMPM).filter(n => n > 0);
  let trendLabel = '';
  if (paces.length >= 3) {
    const half      = Math.floor(paces.length / 2);
    const avgFirst  = paces.slice(0, half).reduce((a, b) => a + b, 0) / half;
    const avgSecond = paces.slice(Math.ceil(paces.length / 2)).reduce((a, b) => a + b, 0) /
                       (paces.length - Math.ceil(paces.length / 2));
  ...
  // Detect HR drift (HR trending up while pace is steady or improving)
  const hrVals = splits.map(s => s.hr).filter(Boolean);
  let hrLabel = '';
  if (hrVals.length >= 3 && paces.length >= 3) {
    const hrFirst  = hrVals.slice(0, Math.floor(hrVals.length / 2));
    const hrSecond = hrVals.slice(Math.ceil(hrVals.length / 2));
    const hrAvgFirst  = hrFirst.reduce((a, b) => a + b, 0) / hrFirst.length;
    const hrAvgSecond = hrSecond.reduce((a, b) => a + b, 0) / hrSecond.length;
    if (hrAvgSecond - hrAvgFirst > 6) hrLabel = ' · HR drift +' + Math.round(hrAvgSecond - hrAvgFirst) + 'bpm';
  }
  ...
  return `\n  Mile splits (Strava${trendLabel}${hrLabel}):\n` + lines.join('\n');
}
